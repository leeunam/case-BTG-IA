...
interface Props { open: boolean; jobId: string; onClose: () => void }

export default function PdfGenerationModal({ open, jobId, onClose }: Props) {
  const { data } = useQuery({
  ...
  return (
    <Modal open={open} onClose={onClose} title="Gerando relatório analítico" width="max-w-sm">
        ...
        {(status === 'queued' || status === 'processing') && (
          <>
            ...
            <p className="text-xs text-gray-400 text-center">
              Você pode continuar usando a aplicação. O download ficará disponível quando pronto.
            </p>
          </>
        )}
